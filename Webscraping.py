...
def webscraping():
    ...
    for i in st.session_state['iaItemListWithLink'].ItemUPC:
        ...
        for j in range(0,st.session_state['number_of_columns']):
            try:
                st.session_state['results'].append(lxml_soup.xpath(st.session_state['xpath_of_columns'][j])[0])
            except:
                st.session_state['results'].append('NAN')
            st.session_state['iaItemListWithLink'][st.session_state['names_of_columns'][j]][st.session_state['iaItemListWithLink']['ItemUPC']==i] = st.session_state['results'][j]
...
file_type = st.selectbox(
                ...
                file_extensions)
if(file_type=='CSV'):
    st.session_state['uploaded_file'] = st.file_uploader("Carga tu archivo",label_visibility="hidden")
    if(st.session_state['uploaded_file'] is not None):
        try:
            st.session_state['iaItemListWithLink'] = pd.read_csv(st.session_state['uploaded_file'], encoding='UTF-8-SIG')
            st.dataframe(st.session_state['iaItemListWithLink'].astype('str'))
        except:
            st.session_state['iaItemListWithLink'] = pd.read_csv(st.session_state['uploaded_file'], encoding='ISO-8859-1')
            st.dataframe(st.session_state['iaItemListWithLink'].astype('str'))
if(file_type=='Excel'):
    st.session_state['uploaded_file'] = st.file_uploader("Carga tu archivo",label_visibility="hidden")
    st.session_state['excel_sheet'] = st.text_input(label='Escribe el nombre o número de la página que contiene los enlaces', value='')
    if(st.session_state['uploaded_file'] is not None and st.session_state['excel_sheet']!=''):
        st.session_state['iaItemListWithLink'] = pd.read_excel(st.session_state['uploaded_file'], sheet_name=st.session_state['excel_sheet'])
        st.dataframe(st.session_state['iaItemListWithLink'].astype('str'))
...
if(st.session_state['iaItemListWithLink'] is not None):
    ...
    if(st.session_state['column_of_code'] != '' and st.session_state['column_of_link']!=''):
        ...
        st.session_state['number_of_columns'] = st.number_input('¿Cuántos datos quieres obtener?', value=0, step=1)
        ...
        if init_ws == True:
            ...
            st.download_button(
                label="Descargar CSV",
                data=csv_file,
                file_name='webscraping_'+ re.sub("\.[a-zA-Z]+$", '',st.session_state['uploaded_file'].name) +'.csv',
                mime='text/csv',
            )
            ...
            st.download_button(
                ...
                data=excel_file,
                file_name='webscraping_'+ re.sub("\.[a-zA-Z]+$", '',st.session_state['uploaded_file'].name) +'.xlsx',
